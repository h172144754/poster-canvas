const main = {
    ...
    async drawImage(data) {
        ...
        this.ctx.save();
        if (borderRadius > 0) {
            const value = await this.loadImage(this.canvas, imgPath)
            this._drawPicRadius(this.toPx(x), this.toPx(y), this.toPx(w), this.toPx(h), this.toPx(borderRadius));

            this.ctx.clip();
            this.ctx.drawImage(value, this.toPx(sx), this.toPx(sy), this.toPx(sw), this.toPx(sh), this.toPx(x), this.toPx(y), this.toPx(w), this.toPx(h));
            if (borderWidth > 0) {
...
const handle = {
    ...
    _drawRadiusRect(x, y, w, h, r = 0, c) {
        ...
        this.ctx.arc(this.toPx(x + br), this.toPx(y + br), this.toPx(br), Math.PI, 1.5 * Math.PI)
        this.ctx.moveTo(this.toPx(x + br), this.toPx(y)); // 移动到左上角的点
        this.ctx.lineTo(this.toPx(x + w - br), this.toPx(y));
        this.ctx.arc(this.toPx(x + w - br), this.toPx(y + br), this.toPx(br), 2 * Math.PI * 1.5, 2 * Math.PI)
        this.ctx.lineTo(this.toPx(x + w), this.toPx(y + h - br));
        this.ctx.arc(this.toPx(x + w - br), this.toPx(y + h - br), this.toPx(br), 0, 0.5 * Math.PI)
        this.ctx.lineTo(this.toPx(x + br), this.toPx(y + h));
        this.ctx.arc(this.toPx(x + br), this.toPx(y + h - br), this.toPx(br), Math.PI * 0.5, Math.PI)
        this.ctx.lineTo(this.toPx(x), this.toPx(y + br));
        this.ctx.fill();
        this.ctx.closePath();
    },
    _drawPicRadius(x, y, w, h, r) {
        if (w < 2 * r) {
            r = w / 2;
        }
        if (h < 2 * r) {
        ...
        // this.ctx.moveTo(this.toPx(x + r ), this.toPx(y));
        // this.ctx.arc(this.toPx(x + r), this.toPx(y + r), this.toPx(r), Math.PI, 1.5 * Math.PI)
        // this.ctx.arc(this.toPx(x + w - r), this.toPx(y + r), this.toPx(r), 2 * Math.PI * 1.5, 2 * Math.PI)
        // this.ctx.arc(this.toPx(x + w - r), this.toPx(y + h - r), this.toPx(r), 0, 0.5 * Math.PI)
        // this.ctx.arc(this.toPx(x + r), this.toPx(y + h - r), this.toPx(r), Math.PI * 0.5, Math.PI)
        this.ctx.closePath();
    },
    /**
     * 计算文本长度
...
const helper = {
    ...
    _downloadImageAndInfo(image, index, pixelRatio) {
        return new Promise((resolve, reject) => {
            ...
            //     // 获取图片信息
            //     .then(imgPath => )
            this._getImageInfo(imageUrl, index).then(({
                    imgPath,
                    imgInfo
                }) => {
    ...
    _downImage(imageUrl) {
        return new Promise((resolve, reject) => {
            if (/^https/.test(imageUrl) && !new RegExp(wx.env.USER_DATA_PATH).test(imageUrl)) {
                wx.downloadFile({
                    url: this._mapHttpToHttps(imageUrl),
                    success: (res) => {
                        if (res.statusCode === 200) {
                            resolve(this._mapHttpToHttps(imageUrl))
    ...
    methods: Object.assign({
        ...
        create(config) {
            ...
            query.select('#canvas').fields({
                ...
                this.canvas.width = this.toPx(config.width) * this.dpr
                this.canvas.height = this.toPx(config.height) * this.dpr
                this.ctx.scale(this.dpr, this.dpr)
            })
            this.pixelRatio = config.pixelRatio || 1;
            const height = this.getHeight(config);
            this.initCanvas(this.toPx(config.width), this.toPx(height), config.debug)
                .then(() => {
                    this.ctx.save();
                    this.ctx.beginPath();
                    this._drawRadiusRect(0, 0, config.width, height, config.borderRadius, config.backgroundColor)
                    this.ctx.clip();
                    this.ctx.restore()
                    // 设置画布底色
